...
const tenantStorage = require('../infrastructure/tenantContext');
 ...
 * - Enregistre le contexte dans AsyncLocalStorage pour le Mongoose tenant plugin.
 */
const tenantMiddleware = (req, _res, next) => {
  ...
  if (req.scope === 'PLATFORM') {
    return tenantStorage.run({ scope: 'PLATFORM', companyId: null }, next);
  }
  ...
  // Propager le contexte tenant à tous les handlers async de cette requête
  tenantStorage.run({ companyId, scope: req.scope || 'COMPANY' }, next);
};
